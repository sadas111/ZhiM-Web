...
interface QuestionState {
  ...
  presetId?: string; // UI preset for fast configuration
  topic: string;
  difficulty: string;
  type: string;
  count: number;
  additionalRequirements?: string; // Extra constraints for generation prompt
  selectedKb: string;
...
const CHINESE_ONLY_CONSTRAINT =
  "请使用中文输出题干、选项与解析，避免使用英文（专有名词/标准缩写如 TCP/IP、HTTP、DNS 或必要的数学符号与公式除外）。";
...
export function GlobalProvider({ children }: { children: React.ReactNode }) {
    ...
    const loadSidebarSettings = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.nav_order) {
            const serverOrder = data.nav_order as Partial<SidebarNavOrder>;
            const mergedOrder: SidebarNavOrder = {
              start: Array.isArray(serverOrder.start)
                ? Array.from(
                    new Set([...serverOrder.start, ...DEFAULT_NAV_ORDER.start]),
                  )
                : DEFAULT_NAV_ORDER.start,
              learnResearch: Array.isArray(serverOrder.learnResearch)
                ? Array.from(
                    new Set([
                      ...serverOrder.learnResearch,
                      ...DEFAULT_NAV_ORDER.learnResearch,
                    ]),
                  )
                : DEFAULT_NAV_ORDER.learnResearch,
            };
            // Filter out removed routes (avoid blank items from stale saved nav_order)
            const removed = new Set<string>(["/ai_lab"]);
            const cleaned: SidebarNavOrder = {
              start: mergedOrder.start.filter((x) => !removed.has(x)),
              learnResearch: mergedOrder.learnResearch.filter(
                (x) => !removed.has(x),
              ),
            };
            setSidebarNavOrderState(cleaned);
          }
        }
      } catch (e) {
  ...
  const startQuestionGen = (
    ...
    setQuestionState((prev) => ({
      ...
      type,
      count,
      additionalRequirements: additionalRequirements ?? prev.additionalRequirements,
      selectedKb: kb,
      progress: {
    ...
    ws.onopen = () => {
      const extraReqRaw = (additionalRequirements || "").trim();
      const extraReq =
        extraReqRaw.length > 0
          ? `${extraReqRaw}\n\n${CHINESE_ONLY_CONSTRAINT}`
          : CHINESE_ONLY_CONSTRAINT;
