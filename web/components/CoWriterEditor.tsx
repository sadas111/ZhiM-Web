        ...
        if (statusRes.ok) {
          ...
          if (statusData.available) {
            ...
            if (voicesRes.ok) {
              ...
              setTtsVoices(voices);
              // If current selected voice is invalid for this provider, fall back safely.
              const ids = new Set(voices.map((v: any) => v.id));
              setSelectedVoice((prev) => {
                if (prev && ids.has(prev)) return prev;
                if (statusData.default_voice && ids.has(statusData.default_voice))
                  return statusData.default_voice;
                return voices[0]?.id || prev || "alloy";
              });
            }
          } else {
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-700 animate-in slide-in-from-top-2">
              ...
              <div className="flex flex-col md:flex-row gap-3">
                <div className="flex-1 space-y-2">
                  {/* Voice selector (if TTS is available) */}
                  {ttsAvailable && ttsVoices.length > 0 && (
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className="flex-1 px-2 py-1.5 text-xs border border-slate-200 dark:border-slate-600 rounded-lg outline-none bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200"
                      >
                        {ttsVoices.map((v) => (
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={async () => {
                        ...
                        try {
                          // Refresh TTS availability at click-time (settings may have changed after mount)
                          let ttsOk = Boolean(ttsAvailable);
                          try {
                            const statusRes = await fetch(
                              apiUrl("/api/v1/co_writer/tts/status"),
                            );
                            if (statusRes.ok) {
                              const statusData = await statusRes.json();
                              ttsOk = Boolean(statusData?.available);
                              setTtsAvailable(ttsOk);
                              if (ttsOk && statusData?.default_voice) {
                                setSelectedVoice(statusData.default_voice);
                              }
                            }
                          } catch {
                            // keep previous ttsAvailable
                          }
                          ...
                          const res = await fetch(
                            apiUrl("/api/v1/co_writer/narrate"),
                            {
                              ...
                              body: JSON.stringify({
                                ...
                                // If TTS is not available, request script-only.
                                skip_audio: !ttsOk,
                                // Prefer server default voice when not explicitly configured in UI.
                          ...
                          if (data.has_audio && data.audio_url) {
                            ...
                            setAudioInfo(null);
                            if (data.audio_error) {
                              setNarrationError(String(data.audio_error));
                            }
                          }
                        } catch (e: any) {
                          setNarrationError(
