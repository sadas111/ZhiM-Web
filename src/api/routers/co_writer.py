...
async def get_tts_status():
    ...
    try:
        ...
        return {
            "available": True,
            "provider": tts_config.get("provider") or "openai",
...
async def get_available_voices():
    ...
    - OpenAI: returns OpenAI voice roles (alloy, echo, ...)
    - Edge TTS: returns a curated set of common voices (offline-friendly)
    """
    provider = "openai"
    try:
        tts_config = get_tts_config()
        provider = (tts_config.get("provider") or "openai").lower()
    except Exception:
        provider = "openai"

    if provider == "edge_tts":
        # Curated voices that work well for zh/en without calling remote voice catalogs.
        voices = [
            {"id": "zh-CN-XiaoxiaoNeural", "name": "晓晓 (女)", "description": "普通话，清晰自然"},
            {"id": "zh-CN-XiaoyiNeural", "name": "晓伊 (女)", "description": "普通话，柔和"},
            {"id": "zh-CN-YunxiNeural", "name": "云希 (男)", "description": "普通话，沉稳"},
            {"id": "zh-CN-YunjianNeural", "name": "云健 (男)", "description": "普通话，成熟"},
            {"id": "zh-CN-YunyangNeural", "name": "云扬 (男)", "description": "普通话，明亮"},
            {"id": "zh-CN-liaoning-XiaobeiNeural", "name": "小北 (女)", "description": "东北口音"},
            {"id": "zh-HK-HiuMaanNeural", "name": "曉曼 (粤语·女)", "description": "粤语"},
            {"id": "zh-HK-WanLungNeural", "name": "雲龍 (粤语·男)", "description": "粤语"},
            {"id": "en-US-JennyNeural", "name": "Jenny (EN·女)", "description": "American English"},
            {"id": "en-US-GuyNeural", "name": "Guy (EN·男)", "description": "American English"},
        ]
