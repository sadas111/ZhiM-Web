...
from src.dt_logging import get_logger
...
# Accept both with/without trailing slash because the app disables redirect_slashes
# (FastAPI won't auto-redirect /upload/ -> /upload when redirect_slashes=False).
...
async def upload_files(
    ...
    rag_provider: Optional[str] = Form(None),
):
    ...
    try:
        ...
        try:
            ...
            # KB can exist in centralized config but its directory may not yet exist on disk
            # (e.g., config restored, partial initialization, or manual config edits).
            # In that case, create the minimal directory structure needed for uploads.
            if kb_name in manager.list_knowledge_bases():
                kb_path = Path(manager.base_dir) / kb_name
        ...
        # Decide whether this KB can accept incremental uploads.
        # DocumentAdder requires an initialized index directory:
        # - llamaindex: llamaindex_storage/
        # - others: rag_storage/
        ...
        if metadata_file.exists():
            try:
                ...
                with open(metadata_file, "r", encoding="utf-8") as f:
                    resolved_provider = (json.load(f) or {}).get("rag_provider")
            except Exception:
                resolved_provider = None

        if not resolved_provider:
            # Fallback to centralized config or request param
            try:
                manager.config = manager._load_config()
                resolved_provider = (
                    manager.config.get("knowledge_bases", {})
                    .get(kb_name, {})
                    .get("rag_provider")
                )
            except Exception:
        ...
        resolved_provider = resolved_provider or rag_provider or "raganything"

        is_initialized = False
        if resolved_provider == "llamaindex":
            is_initialized = (kb_path / "llamaindex_storage").exists()
        else:
            is_initialized = (kb_path / "rag_storage").exists()

        if not is_initialized:
            # KB isn't initialized yet: run full initialization on the uploaded raw files.
            # This avoids "Knowledge base not initialized" errors from incremental add.
            logger.info(
                f"KB '{kb_name}' not initialized yet; running initialization (provider={resolved_provider})"
            )
            try:
            ...
            progress_tracker = ProgressTracker(kb_name, Path(_kb_base_dir))
...
@router.options("/{kb_name}/upload")
@router.options("/{kb_name}/upload/")
async def upload_files_options(kb_name: str):
    # CORS preflight helper; methods list is informational.
    return {"allowed_methods": ["POST", "OPTIONS"], "kb_name": kb_name}
...
@router.options("/create/")
async def create_knowledge_base_options():
    # CORS preflight helper; methods list is informational.
