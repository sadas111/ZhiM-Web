...
async def test_tts_connection():
    ...
    try:
        ...
        if provider != "edge_tts" and (not api_key or not base_url):
            return TestResponse(
                success=False,
                message="TTS configuration incomplete",
                model=model,
        ...
        # For Edge TTS, do a lightweight real synthesis test (local, no paid API calls).
        if provider == "edge_tts":
            try:
                ...
                voice = tts_config.get("voice") or "zh-CN-XiaoxiaoNeural"
                with tempfile.NamedTemporaryFile(suffix=".mp3", delete=True) as tmp:
                    communicate = edge_tts.Communicate("TTS test.", voice)
        ...
        # For paid TTS providers, we only verify config (avoid real API calls).
        response_time = (time.time() - start_time) * 1000

        return TestResponse(
            success=True,
            message=f"TTS configuration valid (provider={provider})",
            model=model,
            response_time_ms=round(response_time, 2),
        )
