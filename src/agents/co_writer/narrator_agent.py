...
class NarratorAgent(BaseAgent):
    ...
    def _validate_tts_config(self):
        ...
        provider = self.tts_config.get("provider") or os.getenv("TTS_BINDING", "openai")
        ...
        if provider != "edge_tts":
            api_key = self.tts_config.get("api_key", "")
            base_url = self.tts_config.get("base_url", "")
            api_key_preview = (
    ...
    async def generate_audio(self, script: str, voice: str = None) -> dict[str, Any]:
        ...
        try:
            binding = (self.tts_config.get("provider") or os.getenv("TTS_BINDING", "openai")).lower()
            ...
            if binding == "edge_tts":
                ...
                # Edge voices look like: zh-CN-XiaoxiaoNeural
