...
from src.dt_logging import Logger, get_logger
...
class AgentCoordinator:
    ...
    async def generate_question(
        ...
        if not retrieval_result.get("has_content"):
            # Fallback: still generate a question without KB context.
            # This is important for first-time users or empty KBs; otherwise the UX feels broken.
            self.logger.warning(
                "No relevant knowledge found; falling back to zero-context generation"
            )
            await self._send_ws_update(
                "log",
                {
                    "level": "WARNING",
                    "module": "QuestionCoordinator",
                    "content": "No relevant knowledge found; generating without KB context (fallback).",
                    "message": "No relevant knowledge found; generating without KB context (fallback).",
                },
            )
            knowledge_context = ""
        else:
    ...
    async def generate_questions_custom(
        ...
        if not retrieval_result.get("has_content"):
            # Fallback: still generate questions even if KB has no relevant hits.
            # This keeps the generator usable without requiring perfect KB coverage.
            self.logger.warning(
                "No relevant knowledge found; falling back to zero-context generation"
            )
            await self._send_ws_update(
