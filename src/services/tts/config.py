...
def get_tts_config() -> dict:
    ...
    try:
        ...
        if config and config.get("model"):
            return {
                "provider": config.get("provider") or os.getenv("TTS_BINDING", "openai"),
                "model": config["model"],
    ...
    provider = _strip_value(os.getenv("TTS_BINDING", "openai")) or "openai"
    ...
    # Edge TTS runs locally; it doesn't require api_key/base_url.
