...
def get_embedding_client(config: Optional[EmbeddingConfig] = None) -> EmbeddingClient:
    ...
    # Always derive an effective config for signature comparison.
    effective_config = config or get_embedding_config()
    sig = "|".join(
        [
            effective_config.binding or "",
            effective_config.model or "",
            (effective_config.base_url or "").rstrip("/"),
            str(effective_config.dim or ""),
        ]
    )

    # Recreate client if config changed (supports runtime Settings updates).
    if _client is None or _client_sig != sig:
        _client = EmbeddingClient(effective_config)
        _client_sig = sig
    return _client
